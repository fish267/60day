...
# 18.[When Sigma1 Function Has Equals Values For an Integer and Its Reversed One](http://www.codewars.com/kata/when-sigma1-function-has-equals-values-for-an-integer-and-its-reversed-one)
...
![source: imgur.com](http://i.imgur.com/PPn4EGC.jpg)
...
def equal_sigma1(nMax):
    ...
    for i in range(nMax + 1):
        if i == reverse_num(i):
            continue
        if sum_factors(i) == sum_factors(reverse_num(i)):
            sum += i
    return sum
```
上面那个函数只能跑通过测试Case， 超过了6000ms时间设置, 需要优化一下

# 19. [Sum consecutives](http://www.codewars.com/kata/sum-consecutives) 
...
# 20. [Number Shortening Filter](http://www.codewars.com/kata/number-shortening-filter)
